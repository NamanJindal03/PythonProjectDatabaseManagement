...
def updation(word):
    global collectioncars
    var1 = cmodelentry.get()
    var2 = ccompanyentry.get()
    var3 = ccolorentry.get()
    var4 = cpriceentry.get()
    var5 = ctypeentry.get()
    for i in range(len(collectioncars)-1):
        if collectioncars[i][0]==var1[0]:
...
update=ttk.Button(bottomframe,width=15,text="UPDATE",command=lambda:updation(collectioncars))
